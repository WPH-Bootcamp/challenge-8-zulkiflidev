    ...
    <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 py-4 lg:grid-cols-3 gap-2 md:gap-8 mt-8 md:mt-12 max-w-6xl mx-auto">
          ...
          {smartITSolutions.map((item, index) => (
              <div key={index} className="bg-gray-100 dark:bg-neutral-900 border border-neutral-900 p-6 
                                          md:p-8 pt-12 md:pt-14 rounded-2xl flex flex-col items-start 
                                          relative h-full mt-8 md:mt-10">
                <img src={getImageUrl(item.icon)} className="absolute -top-7 md:-top-8 left-6 md:left-8 w-14 h-14 md:w-16 md:h-16 
                                                            object-contain shrink-0" alt={item.title} />
