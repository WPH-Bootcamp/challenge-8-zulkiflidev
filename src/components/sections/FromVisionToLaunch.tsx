...
function getImageUrl(imageName: string) {
  return new URL(`../../assets/FromVisionToLaunch/${imageName}.png`, import.meta.url).href;
}
    ...
    <div className="md:px-12">
        ...
        <div className="flex flex-col md:flex-row gap-6 mt-16 max-w-6xl mx-auto px-4 justify-center items-stretch">
            {portfolioProjects.map((item) => (
              <div key={item.id} className="w-full md:w-1/3 rounded-2xl shadow-sm flex flex-col items-start overflow-hidden border border-gray-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 transition-all hover:-translate-y-1 hover:shadow-md">
                <img src={getImageUrl(item.imageName)} alt={item.title} className="w-full aspect-video object-cover" />
