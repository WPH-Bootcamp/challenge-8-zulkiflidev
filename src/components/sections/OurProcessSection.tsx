            ...
            {processSteps.map((step, index) => {
              ...
              return (
                <div key={step.id} className="md:contents relative flex flex-row">
                  ...
                  <div className={`order-2 md:order-none flex-1 md:flex-none md:w-auto z-10 md:py-6 text-left md:text-right ${!isEven ? 'hidden md:block' : ''} ${index === processSteps.length - 1 ? '' : 'pb-16 md:pb-0'}`}>
                    {isEven && (
                  ...
                  <div className="order-1 md:order-none mr-6 md:mr-0 relative w-12 md:w-auto flex flex-col items-center flex-shrink-0">
                    {/* Garis Atas (Sembunyi di index 0) */}
                    <div className={`w-0.5 bg-gray-200 dark:bg-neutral-800 flex-grow ${index === 0 ? 'opacity-0' : ''}`}></div>
                    ...
                    <div className={`w-0.5 bg-gray-200 dark:bg-neutral-800 flex-grow ${index === processSteps.length - 1 ? 'opacity-0' : 'pb-16 md:pb-0'}`}></div>
                  </div>
