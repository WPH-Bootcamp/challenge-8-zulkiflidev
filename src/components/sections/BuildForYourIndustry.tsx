                ...
                {buildForYourIndustry.map((item, index) => (
                    ...
                    >
                        <div className={`w-1.5 h-6 rounded-full shrink-0 transition-colors 
                                            duration-300 ${activeIndex === index ? 'bg-primary-200' : 
                                            'bg-neutral-600 group-hover:bg-neutral-400'}`}></div>
                        
                        <span className={`text-lg font-medium transition-colors duration-300 
                                        ${activeIndex === index ? 
                                            'text-gray-900 dark:text-neutral-25' : 
                                            'text-gray-500 dark:text-neutral-500 group-hover:text-gray-700 dark:group-hover:text-neutral-300'}`}>
                            {item.title}
                        </span>
                    </div>
                ))}
            ...
            <div className="px-4 md:w-2/3">
                ...
                {buildForYourIndustry.map((item, index) => (
                    <div key={index} className={activeIndex === index ? 'block' : 'hidden'}>
                        ...
                        <img className="w-full aspect-[21/9] rounded-xl object-cover 
                                        object-center shadow-sm" 
                                        src={getImageUrl('BuildForYourIndustry', item.imageName)} alt={item.title} />                        
                    </div>
                ))}
            </div>
        </div>
